...
/**
* \file niftkStereoIterativeCirclesCalibration.cxx
* \brief Calibrate stereo camera, using Circles (Dots) features, and
...
int main(int argc, char ** argv)
{
  if (argc < 13)
  {
    std::cerr << "Usage: niftkStereoIterativeCirclesCalibration modelPoints.txt "
  ...
  try
  {
    ...
    for (int i = numberOfArgumentsBeforeImages; i < argc; i++)
    {
      ...
      niftk::TemplateCirclesPointDetector* detector1 =
          new niftk::TemplateCirclesPointDetector(dots, offset,
      ...
      niftk::TemplateCirclesPointDetector* detector2 =
          new niftk::TemplateCirclesPointDetector(dots, offset,
                                                  cv::CALIB_CB_SYMMETRIC_GRID
                                                  | cv::CALIB_CB_CLUSTERING
                                                 );
      ...
      if (i-numberOfArgumentsBeforeImages < numberOfImagesPerSide)
      {
        ...
        dynamic_cast<niftk::TemplateCirclesPointDetector*>(originalImagesLeft.back().first.get())
        ...
        dynamic_cast<niftk::TemplateCirclesPointDetector*>(imagesForWarpingLeft.back().first.get())
      ...
      else
      {
        ...
        dynamic_cast<niftk::TemplateCirclesPointDetector*>(originalImagesRight.back().first.get())
        ...
        dynamic_cast<niftk::TemplateCirclesPointDetector*>(imagesForWarpingRight.back().first.get())
    ...
    std::cout << "niftkStereoIterativeCirclesCalibration:(" << imageSize.width << "," << imageSize.height <<  ") "
