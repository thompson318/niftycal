...
while (change > 1e-9)&(iter < MaxIter),
    ...
    for kk = ind_active, %1:n_ima,
        ...
        if optimization_flag == 1 % Weighted Bundle Adjustment
            % higuchi
            Atmp = A';
            Btmp = B';
            for loop=1 : dx*dy
                if outlier_flag == 0
                    ...
                    sigma = [ weight(1, loop), weight(3, loop); weight(3, loop), weight(2, loop) ];
                elseif outlier_flag == 1
                    if outlier_map(loop) == 0
                        sigma = zeros(2);
                    else
                        sigma = [ weight(1, loop), weight(3, loop); weight(3, loop), weight(2, loop) ];
                    end
                end
                atmp = Atmp((loop*2)-1:loop*2, :);
                atmp = sigma*atmp;
                Atmp((loop*2)-1:loop*2, :)=atmp;

                btmp = Btmp((loop*2)-1:loop*2, :);
                btmp = sigma*btmp;
                Btmp((loop*2)-1:loop*2, :)=btmp;
            end

            JJ3(1:10,1:10) = JJ3(1:10,1:10) + sparse(A*Atmp);%A');%higuchi
            JJ3(15+6*(kk-1) + 1:15+6*(kk-1) + 6,15+6*(kk-1) + 1:15+6*(kk-1) + 6) = sparse(B*Btmp);%B');%higuchi
            %JJ3(1:10,1:10) = JJ3(1:10,1:10) + sparse(A*A');
            %JJ3(15+6*(kk-1) + 1:15+6*(kk-1) + 6,15+6*(kk-1) + 1:15+6*(kk-1) + 6) = sparse(B*B');

            AB = sparse(A*Btmp);%B');%higuhci
            BA = sparse(B*Atmp);%B');%higuhci
            %AB = sparse(A*B');
            JJ3(1:10,15+6*(kk-1) + 1:15+6*(kk-1) + 6) = AB;
            JJ3(15+6*(kk-1) + 1:15+6*(kk-1) + 6,1:10) = BA;%(AB)';%higuchi
            ...
            for loop=1 : dx*dy
                ...
                exwk = exkk(1:2, loop);
                exwk = sigma*exwk;
                exkk(1, loop) = exwk(1,1);
                exkk(2, loop) = exwk(2,1);
            end
        else % Bundle Ajustment without weight
            JJ3(1:10,1:10) = JJ3(1:10,1:10) + sparse(A*A');
            JJ3(15+6*(kk-1) + 1:15+6*(kk-1) + 6,15+6*(kk-1) + 1:15+6*(kk-1) + 6) = sparse(B*B');
    ...
    if recompute_extrinsic,
        ...
        for kk =ind_active, %1:n_ima,
            ...
            % higuchi
            outlier_map = outlier_map_arr(:,kk);
            num_pnt = size(outlier_map);
            num_valid_pnt = sum(outlier_map(:));
            counter=1;
            x_kk_tmp = zeros(2, num_valid_pnt);
            X_kk_tmp = zeros(3, num_valid_pnt);
            for loop_pnt = 1: num_pnt
                if outlier_map(loop_pnt) == 1
                    x_kk_tmp(1, counter) = x_kk(1, loop_pnt);
                    x_kk_tmp(2, counter) = x_kk(2, loop_pnt);
                    X_kk_tmp(1, counter) = X_kk(1, loop_pnt);
                    X_kk_tmp(2, counter) = X_kk(2, loop_pnt);
                    X_kk_tmp(3, counter) = X_kk(3, loop_pnt);
                    counter = counter + 1;
                end
            end
            ...
            [omc_current,Tc_current] = compute_extrinsic_init(x_kk_tmp,X_kk_tmp,fc_current,cc_current,kc_current,alpha_current);
            [omckk,Tckk,Rckk,JJ_kk] = compute_extrinsic_refine(omc_current,Tc_current,x_kk_tmp,X_kk_tmp,fc_current,cc_current,kc_current,alpha_current,MaxIter2,thresh_cond);
            if check_cond,
                if (cond(JJ_kk)> thresh_cond),
                    active_images(kk) = 0;
                    fprintf(1,'\nWarning: View #%d ill-conditioned. This image is now set inactive. (note: to disactivate this option, set check_cond=0)\n',kk);
...
fprintf(1,'Rotation Vec:         omc = [ %3.9f   %3.9f   %3.9f   ] ± \n',[omckk]);   
fprintf(1,'Translation Vec:       Tc = [ %3.9f   %3.9f   %3.9f   ] ± \n',[Tckk]);   
...
%fprintf(1,'      For accurate (and stable) error estimates, it is
%recommended to run Calibration once again.\n\n\n')
fc_buf(1, loopIterate) = fc(1);
fc_buf(2, loopIterate) = fc(2);
cc_buf(1, loopIterate) = cc(1);
cc_buf(2, loopIterate) = cc(2);
kc_buf(1, loopIterate) = kc(1);
kc_buf(2, loopIterate) = kc(2);
kc_buf(3, loopIterate) = kc(3);
kc_buf(4, loopIterate) = kc(4);
kc_buf(5, loopIterate) = kc(5);
%{
omc_buf(1, loopIterate) = omckk(1);
omc_buf(2, loopIterate) = omckk(2);
omc_buf(3, loopIterate) = omckk(3);
tc_buf(1, loopIterate) = Tckk(1);
tc_buf(2, loopIterate) = Tckk(2);
tc_buf(3, loopIterate) = Tckk(3);
%}
pixerror_buf(1, loopIterate) = err_std(1);
pixerror_buf(2, loopIterate) = err_std(2);
