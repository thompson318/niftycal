function [Param] = computeCorrelationSSD(directoryName, current_loop, im,iter_no,dy,dx,filter,distancex,distancey)
...
%         end
        ...
        try
            C = normxcorr2(circle,I);
        catch
            if (size(I,1)>=size(circle,1) & size(I,2)>=size(circle,2))
                C = normxcorr2(I,circle);
            else
                miny=min([size(I,1) size(circle,1)]);
                minx=min([size(I,2) size(circle,2)]);
                C = normxcorr2(I(1:miny,1:minx),circle(1:miny,1:minx));
            end
        end
